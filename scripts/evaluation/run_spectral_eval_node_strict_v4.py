...
Embedding (subgrafo comun, job 308480) en el split node_strict de
GEN-ass-DIS (unica tarea con este split, ver Seccion 3). Mismos
embeddings ya usados para el split "edge" -- no requiere reentrenar
nada, solo evaluar con los pares/negativos *_strict.tsv.

Uso:
    python3 run_spectral_eval_node_strict_v4.py \
        ...
        --out data/processed/evaluation/graph_v4/spectral_results_node_strict.tsv
"""
import argparse
...
def log(msg):
    print(f"[spectral_eval_node_strict] {msg}", flush=True)
...
def main():
    ...
    result = {
        "task": TASK, "split_type": "node_strict", "method": "spectral",
