#!/usr/bin/env python3
"""
Seccion 4 (TFM graph_v4): entrenamiento de GAE y evaluacion en las 7
tareas primarias, usando el mismo common_eval.py que el resto de
metodos (Hadamard+diff -> regresion logistica -> AUPRC/AUROC).

Uso:
    python3 run_gae_v4.py \
...
def log(msg):
    print(f"[run_gae] {msg}", flush=True)
...
def main():
    ...
    embeddings, timings = train_gae(
        ...
        max_epochs=args.max_epochs, patience=args.patience, seed=args.seed, log_prefix="gae",
    )
    log(f"Entrenamiento GAE completo. t={time.time() - t0:.1f}s. "
    ...
    for task in TASKS:
        ...
        results.append({
            "task": task, "split_type": "edge", "method": "gae",
