#!/usr/bin/env python3
"""
Seccion 10 (TFM graph_v4): entrenamiento de GAE con inicializacion de
nodos GEN via UniProt (condiciones uniprot_fixed / uniprot_finetuned),
evaluado en las 7 tareas primarias con el mismo common_eval.py que el
resto de metodos. Analogo a run_gae_v4.py (condicion 'structural'),
pero pasando --uniprot-tsv/--uniprot-mode a train_gae (ver gae_common.py).

Persiste, ademas de lo que ya guarda run_gae_v4.py:
  - gae_uniprot_{mode}_initial_embeddings.npz: vectores UniProt
  ...
  - gae_training_history.json: igual que en run_gae_v4.py, pero SIN
    los propios vectores iniciales (no son serializables a JSON de
    forma directa -- van en el .npz de arriba); si contienen solo
    metadata escalar/serie temporal (loss, val_auprc, uniprot_mode,
    n_uniprot_covered, etc).

Uso:
    python3 run_gae_uniprot_v4.py \
...
from gae_common import train_gae
...
def log(msg):
    print(f"[run_gae_uniprot] {msg}", flush=True)
...
def main():
    ...
    t0 = time.time()
    embeddings, timings = train_gae(
        args.train_graph, dim=args.dim, hidden_dim=args.dim,
        max_epochs=args.max_epochs, patience=args.patience, seed=args.seed,
        log_prefix=f"gae_uniprot_{args.uniprot_mode}",
        uniprot_tsv=args.uniprot_tsv, uniprot_mode=args.uniprot_mode,
    )
    log(f"Entrenamiento GAE (uniprot_{args.uniprot_mode}) completo. t={time.time() - t0:.1f}s. "
    ...
    np.savez(f"{args.out_dir}/gae_embeddings.npz",
    ...
    if initial_emb:
        np.savez(f"{args.out_dir}/gae_uniprot_{args.uniprot_mode}_initial_embeddings.npz",
        ...
        log(f"Vectores iniciales UniProt proyectados guardados "
            f"({len(initial_emb):,} nodos) -> gae_uniprot_{args.uniprot_mode}_initial_embeddings.npz")

    # timings ya no contiene arrays de numpy tras el pop() de arriba -> JSON-safe
    with open(f"{args.out_dir}/gae_training_history.json", "w") as f:
    ...
    for task in TASKS:
        ...
        results.append({
            "task": task, "split_type": "edge", "method": f"gae_uniprot_{args.uniprot_mode}",
    ...
    out_df.to_csv(f"{args.out_dir}/gae_results.tsv", sep="\t", index=False)
    log(f"Resultados -> {args.out_dir}/gae_results.tsv")
