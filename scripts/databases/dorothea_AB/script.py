#!/usr/bin/env python3
"""
dorothea_AB -- script.py
Procesado de DoRothEA niveles A y B via OmniPath interactions.
...
Output:
  processed/dorothea_AB/GEN-reg-GEN.tsv
  processed/dorothea_AB/GEN-upr-GEN.tsv
  processed/dorothea_AB/GEN-dwr-GEN.tsv
  processed/dorothea_AB/stats.tsv
...
def load_reviewed_uniprot(path: str) -> set:
    ...
    print(f'[dorothea_AB] UniProt revisados: {len(reviewed):,}')
    return reviewed
...
def load_uniprot2ensg(path: str) -> dict:
    ...
    print(f'[dorothea_AB] Mapeo UniProt->ENSG: {len(mapping):,} entradas')
    return mapping
...
def process(raw_dir: str, out_dir: str, reviewed: set, up2ensg: dict):
    ...
    print(f'[dorothea_AB] Cargando {infile} ...')
    ...
    # Filtrar niveles A y B (igual que Bioteque: busca si el nivel esta en el string)
    levels = set()
    ...
    for name, pairs in [('GEN-reg-GEN', reg), ('GEN-upr-GEN', upr), ('GEN-dwr-GEN', dwr)]:
        ...
        with open(path, 'w') as f:
            ...
            for e1, e2 in sorted(pairs):
                f.write(f'{e1}\t{e2}\n')
        print(f'[dorothea_AB] {name}: {len(pairs):,} -> {path}')
    ...
    print('[dorothea_AB] Procesado completado.')
...
def main():
    ...
    sys.stderr.write('[dorothea_AB] Done!\n')
