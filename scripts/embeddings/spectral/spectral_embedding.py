#!/usr/bin/env python
"""
Spectral embedding baseline para el grafo unificado de TFM-NetActivity.

Calcula el Laplaciano normalizado del grafo (no dirigido, no ponderado)
y obtiene los k autovectores de menor autovalor via LAPACK denso.

Nota sobre el metodo: se descarta deliberadamente ARPACK sparse
shift-invert. Dos motivos: (1) el Laplaciano siempre tiene un autovalor
exacto en 0, lo que hace singular la matriz (L - sigma*I) cuando
sigma=0, rompiendo la factorizacion shift-invert por definicion; (2) el
grafo tiene densidad ~5% (110M aristas / 65722 nodos), demasiado denso
para que el metodo sparse aporte ventaja real sobre el denso. Se va
directo a LAPACK denso (eigh con subset_by_index), con coste de
memoria predecible.

Uso:
    python spectral_embedding.py --edges <path> --out-dir <path> --k 256
"""
import argparse
import os
import time
...
def build_adjacency(node1, node2, out_dir):
    ...
    node_order_path = os.path.join(out_dir, "node_order.tsv")
...
def main():
    ...
    parser.add_argument("--edges", required=True, help="Path a graph_edges.tsv")
    ...
    emb_path = os.path.join(args.out_dir, f"spectral_embeddings_k{k}.npy")
    eig_path = os.path.join(args.out_dir, f"spectral_eigenvalues_k{k}.npy")
