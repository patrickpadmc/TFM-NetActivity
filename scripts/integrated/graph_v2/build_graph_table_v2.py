#!/usr/bin/env python3
"""
build_graph_table_v2.py
Version separada de build_graph_table.py para comparar resultados de
embeddings: incluye los 17 databases originales MAS ctdchemgen
(CPD-upe-GEN, CPD-dwe-GEN, CPD-afe-GEN -- interacciones quimico-genicas
que afectan expresion, via CTD).

Construye la tabla base para el grafo de conocimiento biologico.

Output:
  data/processed/graph_v2/graph_edges.tsv     -- todas las aristas
  data/processed/graph_v2/graph_summary.html  -- resumen + muestra 1000 filas
...
Tipos de nodo:
  ...
  CPD -- compuesto (CTD ID o pert_iname o InChIKey)
  PWY -- pathway (R-HSA-)
  CLL -- linea celular (CVCL)

Uso:
  python build_graph_table_v2.py [--base-dir PATH] [--out-dir PATH]
"""
...
RELATIONS = [
    ...
    # ctdchemgen (nuevo en esta version v2, solo expression)
...
def main():
    args     = parse_args()
    base     = args.base_dir
    proc_dir = f"{base}/data/processed/databases"
    out_dir  = args.out_dir or f"{base}/data/processed/integrated/graph_v2"
    os.makedirs(out_dir, exist_ok=True)

    print("=" * 60)
    print(" TFM-NetActivity -- build_graph_table_v2.py (con ctdchemgen)")
    print("=" * 60)
